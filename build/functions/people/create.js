    ...
    try {
        ...
        const response = await prisma.$transaction(async (prismaTransaction) => {
            const pessoa = await prismaTransaction.pessoa.create({
                data: Object.assign(Object.assign(Object.assign(Object.assign({ codigo: pessoaDTO.codigo, nome: pessoaDTO.nome, sobrenome: pessoaDTO.sobrenome, cpf: pessoaDTO.cpf.replace(/[^\d]+/g, ''), datanascimento: new Date(pessoaDTO.datanascimento), sexo: pessoaDTO.sexo }, (pessoaDTO.tipofisicoid && { tipofisicoid: pessoaDTO.tipofisicoid })), (pessoaDTO.nivelatividadeid && { nivelatividadeid: pessoaDTO.nivelatividadeid })), (pessoaDTO.objetivoid && { objetivoid: pessoaDTO.objetivoid })), { situacao: pessoaDTO.situacao, tipopessoaid: tipoPessoa })
            });
